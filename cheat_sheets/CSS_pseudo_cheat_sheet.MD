...
|`[attribute\|=value]`|`[\|=en] {}`|Elements with lang attribute value equal to `en` or `en-`|
...
|`:required`|`input:required {}`|Selects input elements with `required` attribute specified|
